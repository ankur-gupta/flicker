...
from pyspark.sql.types import (BooleanType, IntegerType, DoubleType, StringType, LongType, FloatType, TimestampType,
                               ArrayType)
from pyspark.sql.functions import udf


def make_double_udf(fun: Callable):
    return udf(fun, DoubleType())


def make_integer_udf(fun: Callable):
    return udf(fun, IntegerType())


def make_string_udf(fun: Callable):
    return udf(fun, StringType())


def make_boolean_udf(fun: Callable):
...
def _row_value(row) -> Any:
    """ A function to extract the contents of the 'value' key in a Row
        ...
        If there are no keys or there are keys but no `value` key, we want this function to fail.
    """
...
def _keys(e: Any) -> list:
    """ A function to extract the keys (not values) from a dict object. If there is no `keys` method, we want this
    function to fail.
    """

    # Ideally, we would've liked to return a `frozenset` type object because it's immutable and dictionary keys are not
    # guaranteed to be ordered. But, there is no pyspark.sql.types.FrozenSetType. So, we have to return python `list`
    # which has a corresponding pyspark.sql.types.ArrayType.

    # We still sort the keys just to have more predictability.
