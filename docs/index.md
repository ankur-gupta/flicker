...
performance at all. [FIXME: see design notes, pyspark vs flicker tutorial].
...
## Quick Example
This example should give you a taste of how to use `flicker`. 
[FIXME: tutorial, flicker vs pyspark]
[FIXME: link to this example in jupyter notebook]

We will assume that you have `flicker` installed and ready to use. 
Please see [Getting Started](http://localhost:8001/getting-started/installation/).
...
wraps the PySpark dataframe within itself[^3] (you can access it at `df._df`).
...
dataframes for the same operations.
...
(such as Jupyter or IPython)[^4]. This also means that if you wanted to 
...
Flicker provides a pandas-like API though PySpark's API is not too bad 
for this either. 
...
This is why we call `df.names` instead of `df.columns`.
Getting the data types of all the columns is the same in Flicker and PySpark.
...
#### Extracting a column
Unlike `pandas.Series` object, the `pyspark.sql.Column` object does not 
...
#### Filtering
This is the also same in Flicker, PySpark, and pandas.   
...
an operation that can easily be performed with PySpark[^5]. You can also 
...
[^3]: [Composition](https://en.wikipedia.org/wiki/Object_composition) is the fancy term for it.
[^4]: Conversion to a pandas dataframe can sometimes convert `np.nan` into `None`.
[^5]: If possible, please contribute by [filing a GitHub issue](https://github.com/ankur-gupta/flicker/issues/new) and/or sending a PR! 
